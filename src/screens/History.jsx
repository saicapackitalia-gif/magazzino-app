  ...
  useEffect(() => {
    async function carica() {
      const { data, error } = await supabase
        .from("movimenti")
        .select("id, tipo_movimento, numero_bancali, numero_scatole, creato_il, bancali(codice_materiale)")
    ...
    <div>
      ...
      <div style={{ padding: 16, display: "flex", flexDirection: "column", gap: 10 }}>
        ...
        {righe.map((r) => {
          ...
          return (
            <div key={r.id} style={{ background: "#fff", borderRadius: 14, padding: 16, display: "flex", gap: 14, alignItems: "center", boxShadow: "0 1px 6px rgba(28,43,51,0.08)" }}>
              <div style={{ width: 8, height: 40, borderRadius: 4, background: etichetta.colore }} />
              <div>
                <div style={{ fontSize: 15, fontWeight: 700, color: "#1C2B33" }}>{etichetta.testo}</div>
